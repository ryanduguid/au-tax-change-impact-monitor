...
The intended future source is a reviewed, deliberately versioned observation output from [Commonwealth tax legislation corpus builder](https://github.com/ryanduguid/SirArthurFadden). That corpus’s distinction between a superseded compilation, a current version with no published compilation, a title no longer in force, and a failed lookup is preserved here. This project is not a replacement corpus builder and must not treat derived corpus material as authorised legal text.
